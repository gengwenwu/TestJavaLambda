...
class Test1 {
	...
	private static void testTypeInfer() {
		// 在Lambda表达式中无需指定类型，程序依然可以编译。
		// 这是因为javac根据程序的上下文在后台推断出了参数的类型。
	...
	// Lambda用在哪里。
	// 每一个Lambda都能通过一个特定的"函数式接口"与一个给定的类型进行匹配。
	// 因此一个Lambda表达式能被应用在与其目标类型匹配的任何地方，lambda表达式必须和函数式接口的抽象函数描述一样的参数类型，
